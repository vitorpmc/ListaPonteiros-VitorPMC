...
// Funcao principal
int main(){
  float initT1, finalT1, initT2, finalT2, *x, *y, t; // declara os ponteiros para inteiro x e y que irao apontar para os valores digitados pelo usuario e a variavel de armazenamento temporario dos valores digitados pelo usuario t, bem como as vairaveis que irao registrar o inicio e final das contagens de tempo 1 e 2
  ...
  initT2 = clock(); // variavel de registro do inicio de tempo de execucao 1 recebe numero de ciclo presente
  y = malloc(n * sizeof(float)); // realiza a alocacao dinamica propria da biblioteca libgc, que apenas necessita do malloc e realiza a liberacao de memoria automaticamente
  ...
  finalT1 = clock();
  finalT2 = clock();
  printf("\nTempo de execucao1 = %.10f", (finalT1 - initT1)/CLOCKS_PER_SEC);
  printf("\nTempo de execucao2 = %.10f", (finalT2 - initT2)/CLOCKS_PER_SEC);
  return 0;
} // FIM DO CODIGO
